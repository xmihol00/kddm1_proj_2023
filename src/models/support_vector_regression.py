...
if __name__ == "__main__":
    ...
    if PERFORM_CROSS_VALIDATION:
        y_test.clear()
        y_pred.clear()
        seed_range = range(40, 50)

        for current_seed in seed_range:
            train_mean = pd.read_csv(DATA_PATH["numeric"] + "Universities_train_mean_"+str(current_seed)+".csv")
            test_mean = pd.read_csv(DATA_PATH["numeric"] + "Universities_test_mean_"+str(current_seed)+".csv")

            train_median = pd.read_csv(DATA_PATH["numeric"] + "Universities_train_median_"+str(current_seed)+".csv")
            test_median = pd.read_csv(DATA_PATH["numeric"] + "Universities_test_median_"+str(current_seed)+".csv")

            train_mixed = pd.read_csv(DATA_PATH["numeric"] + "Universities_train_mixed_"+str(current_seed)+".csv")
            test_mixed = pd.read_csv(DATA_PATH["numeric"] + "Universities_test_mixed_"+str(current_seed)+".csv")
            ...
            seed.extend([current_seed, current_seed])
            ...
            current_frame = result[result['seed'] == current_seed]
            print(current_frame)
    ...
    if PERFORM_CROSS_VALIDATION:
        predicted_truth.to_csv("results/SVR_predicted_truth_SV.csv", index=False)
    else:
